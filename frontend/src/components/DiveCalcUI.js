...
function DiveCalcUI()
{
    // Variables to capture form inputs.
    let firstDive;
    let firstTotalTime;
    ...
    // Helps with changing text colors for invalid inputs.
    ...
    const doReset = async event => 
    {
        ...
        firstDive.value = "";
        firstTotalTime.value = "";
        lenSurfaceIntervalHours.value = "";
        lenSurfaceIntervalMins.value = "";
        secondDive.value = "";
    ...
    const doCalc = async event => 
    {
        ...
        // Vars for user values.
        var depth = parseInt(firstDive.value);
        var time = parseInt(firstTotalTime.value);
        var surface_hr = parseInt(lenSurfaceIntervalHours.value);
        var surface_min = parseInt(lenSurfaceIntervalMins.value);
        var depth2 = parseInt(secondDive.value);
        ...
        {
            errorMessage.push(<span key={1}>Your first dive must be between 1 and 140 ft.<br /></span>);
            setTextColor1("red");
        }
        if (time <= 0)
        {
            errorMessage.push(<span key={2}>You must enter the total time of your first dive.<br /></span>);
            setTextColor2("red");
        }
        if (surface < 10 || surface > 1440)
        {
            errorMessage.push(<span key={3}>Your surface interval must at least 10 minutes and no greater than 24 hours.<br /></span>);
            setTextColor3("red");
        }
        if (depth2 <= 0 || depth2 > 130)
        {
            errorMessage.push(<span key={4}>Your second dive must be between 1 and 130 ft.<br /></span>);
            setTextColor4("red");
        }
        if (errorMessage.length)
        {
            setMessage(<center style={{color:"red"}}>{errorMessage}</center>);
            return;
        }
        ...
        if (first_group == "")
        {
            errorMessage.push(<span key={5}>The time/depth of your first dive is beyond<br />the no-decompression table limits.<br /><br />Please enter a shallower depth and/or shorter dive time.</span>);
            setMessage(<center style={{color:"red"}}>{errorMessage}</center>);
            setTextColor1("red");
            setTextColor2("red");
            return;
        ...
        if (!rnt)
        {
            errorMessage.push(<span key={6}>Avoid repetetive dives over 100 ft.</span>);
            setMessage(<center style={{color:"red"}}>{errorMessage}</center>);
            return;
        }
        ...
        // Final Answer.
        finalAnswer.push(<span key={7}>Pressure group after the first dive is {first_group.toUpperCase()}.<br />Pressure group after the surface interval is {second_group.toUpperCase()}.</span>);

        if (rnt && amdt)
        {
            finalAnswer.push(<span key={8}><br />Residual Nitrogen Time is {rnt} minutes.<br />Adjusted maximum no-decompression bottom time is {amdt} minutes.</span>);
        }
        else if (rnt)
        {
            finalAnswer.push(<span key={9}><br />Residual Nitrogen Time is {rnt} minutes.<br />No second no-decompression dive is possible.</span>);
        }
        else
        {
            finalAnswer.push(<span key={10}><br />No limit.</span>);
        }
        ...
        {
            finalAnswer.push(<span key={11}><br /><br />NOTE: <i>Safety stop required after first dive</i>.</span>);
        }
        if ((first_group == "w") || (first_group == "x"))
        {
            finalAnswer.push(<span key={12}><br /><br />NOTE: <i>Minimum surface interval is 1 hour (WXYZ rule)</i>.</span>);
        }
        if ((first_group == "y") || (first_group == "z"))
        {
            finalAnswer.push(<span key={13}><br /><br />NOTE: <i>Minimum surface interval is 3 hours (WXYZ rule)</i>.</span>);
        }

        setMessage(<center><b>{finalAnswer}</b></center>);
        return;
    }
        ...
        <div id="diveCalcUIDiv">
            <form className="diveCalcForm" onSubmit={doCalc}>
                <div className="diveCalcInputs">
                    <div className="firstDive">
                        <div className="firstDiveDiv">
                            <label htmlFor="calcFirstDive" style={{color: textColor1}}>
                                Depth of First Dive
                            </label>
                            <input type="text" 
                    ...
                    <div className="firstTotalTime">
                        <div className="firstTotalTimeDiv">
                            <label htmlFor="calcFirstTotalTimeDive" style={{color: textColor2}}>
                    ...
                    <div className="lenSufaceInterval">
                        <div className="surfaceIntervalHourDiv">
                            <label htmlFor="surfaceIntervalHours" style={{color: textColor3}}>
                        ...
                        <div className="surfaceIntervalMinsDiv">
                            <label htmlFor="surfaceIntervalMins" style={{color: textColor3}}>
                    ...
                    <div className="secondDive">
                        <div className="secondDiveDiv">
                            <label htmlFor="calcSecondDive" style={{color: textColor4}}>
                ...
                <div className="diveCalcDivBottom">
                    <input type="submit" id="calcButton" className="buttons" value = "Calculate"
                        onClick={doCalc} /><br />
